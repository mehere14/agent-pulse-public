...
class GoogleProvider {
    ...
    async generate(system, prompt, files, tools, config, output_schema, onToken) {
        ...
        if (tools && tools.length > 0) {
            googleTools = [{
                    functionDeclarations: tools.map(t => ({
                        ...
                        parameters: (0, zod_1.toJSONSchema)(t.parameters)
                    }))
                }];
        }
        // 2. Prepare Config
        ...
        if (output_schema) {
            ...
            generateConfig.responseSchema = (0, zod_1.toJSONSchema)(output_schema);
        }
        // 3. Prepare Contents
        let contents;
        if (Array.isArray(prompt)) {
            ...
            if (files && files.length > 0) {
                ...
                if (lastMsg && lastMsg.role === 'user') {
                    const hasImages = files.some(f => (0, file_utils_1.isImageFile)(f));
                    if (hasImages) {
                        for (const file of files) {
                            if ((0, file_utils_1.isImageFile)(file)) {
                                try {
                                    const base64Data = (0, file_utils_1.readImageFile)(file);
                                ...
                                const textContent = (0, file_utils_1.readMarkdownFiles)([file]);
                                lastMsg.parts.push({ text: `\n\nReference Context:\n${textContent}` });
                            }
                        }
                    }
                    else {
                        const textContent = (0, file_utils_1.readMarkdownFiles)(files);
                        lastMsg.parts.push({ text: `\n\nReference Context:\n${textContent}` });
                    }
                }
            }
